class Intervalo {
  private _limite_inferior: number
  private _limite_superior: number
  private _marca_de_clase: number
  private _frecuencia: number
  private _frecuencia_acumulada: number
  private _frecuencia_POR_marca_de_clase: number

  constructor(li: number, ls: number, fi: number) {
    if (li >= ls) {
      throw new Error(
  ...
  get limite_inferior(): number {
    return this._limite_inferior
  }

  get limite_superior(): number {
    return this._limite_superior
  }

  get marca_de_clase(): number {
  ...
  get frecuencia_acumulada(): number {
    return this._frecuencia_acumulada
  }

  setFrecuenciaAcumulada(fa: number) {
    this._frecuencia_acumulada = fa
  }

  get frecuencia_POR_marca_de_clase(): number {
...
class TablaFrecuencias {
  ...
  private configurarFrecuenciaAcumulada() {
    this.intervalos.reduce(
      (acc, i) => (
        i.setFrecuenciaAcumulada(acc + i.frecuencia), i.frecuencia_acumulada
      ),
      0
    )
  }
  ...
  get amplitud(): number {
    return (
      this.intervalos[0].limite_superior - this.intervalos[0].limite_inferior
    )
  }
  ...
  get media(): number {
    const sumatoria = this.intervalos.reduce(
      (acc, i) => acc + i.frecuencia_POR_marca_de_clase,
      0
    )
  ...
  get moda(): number {
    ...
    const intervaloModa = this.intervalos.find(
      (i) => i.frecuencia === maxFrecuencia
    ) // --> encuentra el intervalo con la frecuencia máxima

    if (!intervaloModa) throw new Error('No se ha encontrado la moda')
    ...
    const f2 =
      i === this.intervalos.length - 1 ? 0 : this.intervalos[i + 1].frecuencia
    const h = this.amplitud
    const moda =
      intervaloModa.limite_inferior + (h * (f1 - f0)) / (f1 - f0 + (f1 - f2))
  ...
  get mediana(): number {
    ...
    const intervaloMediana = this.intervalos.find(
      (i) => i.frecuencia_acumulada >= posicionMediana
    )
    if (!intervaloMediana) throw new Error('No se ha encontrado la mediana')
  ...
  get varianza(): number {
    ...
    const sumatoriaVarianza = this.intervalos.reduce(
      (acc, i) => acc + i.frecuencia * Math.pow(i.marca_de_clase - media, 2),
      0
    )
    return sumatoriaVarianza / n
  }

  get desviacionEstandar(): number {
    return Math.sqrt(this.varianza)
  }

  get coeficienteVariacion(): number {
    return (this.desviacionEstandar / this.media) * 100
  }

  get desviacionMedia(): number {
    const sumatoria = this.intervalos.reduce(
      (acc, i) => acc + i.frecuencia_POR_marca_de_clase,
      0
    )
    const n = this.intervalos.reduce((acc, i) => acc + i.frecuencia, 0)
    const media = sumatoria / n
    const sumatoriaDesviacionMedia = this.intervalos.reduce(
      (acc, i) => acc + i.frecuencia * Math.abs(i.marca_de_clase - media),
      0
    )
  ...
  cuartil(pos: number) {
    ...
    const intervaloCuartil = this.intervalos.find(
      (i) => i.frecuencia_acumulada >= Nq
    )
    if (!intervaloCuartil) throw new Error('No se ha encontrado el cuartil')
  ...
  percentil(pos: number) {
    const Np = (this.N * pos) / 100
    const intervaloCuartil = this.intervalos.find(
      (i) => i.frecuencia_acumulada >= Np
    )
    if (!intervaloCuartil) throw new Error('No se ha encontrado el cuartil')
    const i = this.intervalos.indexOf(intervaloCuartil)
    const li = intervaloCuartil.limite_inferior
